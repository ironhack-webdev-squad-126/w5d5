...
router.get('/roomsAll', rolesCheck('mod'), (req, res, next) => {
  Room.find({})
    .then(rooms => {
      res.render('protected/rooms', { rooms });
    })
...
router.get('/users/edit', rolesCheck('admin'), (req, res, next) => {
  User.find({ role: { $ne: 'admin' } }).then(users => {
    res.render('protected/users-edit', { users });
  });
});

router.post('/users/edit/:userId', rolesCheck('admin'), (req, res, next) => {
  User.updateOne({ _id: req.params.userId }, { role: req.body.role }).then(
    () => {
